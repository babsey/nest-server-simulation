...
def simulate(data, local_num_threads=1):
    ...
    n = nrow*ncol
    neuron = nest.Create(
        nodes[1]['model'], int(n), params=paramify.simulate(nodes[1]))
    ...
    for ii in range(n):
        targets, delay = lcrn.lcrn_gamma_targets(
            ii, nrow, ncol, nrow, ncol, int(outdegree), 4, 3)
        nest.Connect(
            [neuron[ii]], (targets+offset).tolist(), conn_spec=conn_spec,
            syn_spec=syn_spec)
